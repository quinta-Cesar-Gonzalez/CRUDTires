...
class TireBase(BaseModel):
    ...
    layers: Optional[float] = Field(0, ge=0)
    max_pressure: Optional[float] = Field(0, ge=0)
    min_pressure: Optional[float] = Field(0, ge=0)
    max_depth: Optional[float] = Field(0, ge=0)
    min_depth: Optional[float] = Field(0, ge=0)
    wear_type: Optional[str] = Field(None, max_length=50)
    profitability: Optional[float] = Field(0, ge=0)
    performance: Optional[float] = Field(0, ge=0)
    temperature: Optional[str] = Field(None, max_length=50)
    speed: Optional[str] = Field(None, max_length=50)
    speed_number: Optional[float] = Field(0, ge=0)
    braking: Optional[str] = Field(None, max_length=50)
    load_type: Optional[str] = Field(None, max_length=50)
    load_value: Optional[float] = Field(0, ge=0, serialization_alias='_load')
